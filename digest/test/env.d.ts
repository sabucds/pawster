...
declare global {
  namespace Cloudflare {
    interface Env {
      ...
      /**
       * Signs both subscriber links — unsubscribe and manage. A secret for the same reason
       * `RESEND_API_KEY` is, so it is absent from `wrangler.jsonc` and supplied as a fixed
       * string under test. `web/` binds the same one, because it verifies what this Worker
       * signs.
       */
