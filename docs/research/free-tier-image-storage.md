...
> **Correction (2026-09-02, while provisioning — [#18](https://github.com/sabucds/pawster/issues/18)).** The partial-setup escape above is **not available to us.** Cloudflare's DNS docs state that *"a CNAME setup (partial) is only available to customers on a Business or Enterprise plan"*, and list Free and Pro as `No`. A partial setup is therefore reachable only from a plan that costs orders of magnitude more than this project's entire budget, so on the Free plan a **full setup — nameservers delegated to Cloudflare — is the only route to an R2 custom domain.** The conclusion of this section survives intact (custom domain, not `r2.dev`); only the claim that nameservers can stay put is wrong. Practically this costs nothing, because the domain is bought fresh for Pawster and has no existing DNS to migrate — registering it at Cloudflare Registrar creates the zone as a full setup outright. See [`docs/adr/0011-r2-custom-domain-requires-a-full-zone.md`](../adr/0011-r2-custom-domain-requires-a-full-zone.md).
