...
a shared phone, and a credential set once by someone who has since left. So the nudge carries a
**signed confirmation link** that grants a narrow, reversible capability — confirm-all, plus
marking an individual animal `Adopted` or `NoLongerAvailable` — and **never a session**. The
shelter's notification address becomes its confirmation credential, exactly as
...
- **The capability boundary is the point of this ADR.** A confirmation link may confirm and may
  change an animal's state. It may **not** touch the shelter's public contact details — rewriting
  a WhatsApp number silently redirects every adopter enquiry to a stranger and nothing on the page
  would look wrong — nor photos, descriptions, new animals, or the notification address itself.
  Those stay behind a session. The confirm page links out to the login screen; it does not bypass
  it.
- **The privilege ordering is counter-intuitive, so don't "simplify" it.** Confirm-all looks like
  ...
  while a stranger confirming forty stale animals corrupts the one signal the design rests on,
  silently and unfalsifiably.
- **A confirmation link must never mutate on `GET`.** Outlook Safe Links, corporate mail scanners
  ...
  state and every write is a `POST` from it. This is why there is no true "one tap" confirm.
- **Links are reusable and expire by supersession**: each nudge retires the previous link, and
  changing the notification address invalidates every outstanding one. There is no revoke button.
  This requires server-side state — the newest nudge per shelter, keyed to the current notification
  address — so a purely stateless signed payload cannot express this scope.
- **Forwarding is delegation, not a leak to be prevented.** A bearer link is a bearer link;
  ...
  shelter delegates confirmation at zero user-management cost — a future shelter-auth decision
  should not undo it by inventing staff accounts.
